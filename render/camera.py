...
class Camera:
    ...
    def __init__(self, fov=60.0, aspect_ratio=1.0, near=1.0, far=100.0):
        ...
        Keyword Args:
            ...
            far (float): Far plane
        """
        self.position = Vector3([0.0, 0.0, -1.0])
    ...
    def look_at(self, vec=None, pos=None) -> np.ndarray:
    ...
    def _gl_look_at(self, pos, target, up) -> np.ndarray:
...
class OrbitCamera(Camera):
    ...
    def __init__(
        self, target=(0.0, 0.0, 0.0), radius=2.0, angles=(180.0, 90.0), **kwargs
    ):
        """Initialize the camera
    ...
    def matrix(self) -> np.ndarray:
        ...
        position = (
            sin((self.angle_y) * (np.pi/180)) * cos((self.angle_x) * (np.pi/180)) * self.radius + self.target[0],
            cos((self.angle_y) * (np.pi/180)) * self.radius + self.target[1],
            sin((self.angle_y)* (np.pi/180)) * sin((self.angle_x) * (np.pi/180)) * self.radius + self.target[2],
        )
        self.set_position(*position)
        return self.my_create_look_at(
            position,
    ...
    def rot_state(self, dx: float, dy: float) -> None:
        ...
        self.angle_y = max(min(self.angle_y, 179.0), 1.0)
